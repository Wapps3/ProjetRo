...
Welsh_Powell2 = function(X,A){
  ...
  d=rowSums(A)    #somme de chaque lignes de A (les degrés)
  s=sort(d,decreasing=TRUE,index.return=TRUE)
      #Permet de trier la liste d
  k = 0 #Permet de dire sur quel couleur on est
  B = s$ix  #Liste des sommets triés
  
  lignenoncoloree = B
  c=list()
  
  while (length(lignenoncoloree)>0){  #Tant qu'il y a des sommets pas coloriés
    k=k+1
    temp=c()
    
    while(length(B)>0){ 
      temp[length(temp)+1]=B[1] #On prend le premier non colorié
      lignenoncoloree = setdiff(lignenoncoloree,B[1])
      #On l'enlève des sommets non coloriés
      
      if (is.matrix(A[,temp])){   #Si on a une matrice
        B=which(rowSums(A[,temp])==0) 
        #B=liste des lignes non coloriés ayant un zéro dans toutes les
      ...
      #On enlève les nouvelles lignes coloriés
